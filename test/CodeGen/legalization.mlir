...
// CHECK-SAME:                    %[[ARG0:.*]]: !ptr.ptr<#amdgcn.addr_space<global, read_write>>) -> vector<2x2xf32> {
// CHECK:           %[[LOAD0:.*]] = ptr.load %[[ARG0]] : !ptr.ptr<#amdgcn.addr_space<global, read_write>> -> vector<2xf32>
...
// CHECK:           %[[LOAD1:.*]] = ptr.load %[[PTR]] : !ptr.ptr<#amdgcn.addr_space<global, read_write>> -> vector<2xf32>
// CHECK:           %[[TO1:.*]]:2 = vector.to_elements %[[LOAD1]] : vector<2xf32>
// CHECK:           %[[FROM:.*]] = vector.from_elements %[[TO0]]#0, %[[TO0]]#1, %[[TO1]]#0, %[[TO1]]#1 : vector<2x2xf32>
// CHECK:           return %[[FROM]] : vector<2x2xf32>
// CHECK:         }
func.func @load(%m : memref<4x4xf32, #amdgcn.addr_space<global, read_write>>) -> vector<2x2xf32> {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 0 : index
  %v = vector.load %m[%c0, %c1] : memref<4x4xf32, #amdgcn.addr_space<global, read_write>>, vector<2x2xf32>
  return %v : vector<2x2xf32>
}

// CHECK-LABEL:   func.func @store(
// CHECK-SAME:                     %[[ARG0:.*]]: !ptr.ptr<#amdgcn.addr_space<global, read_write>>, %[[ARG1:.*]]: vector<2x2xf32>) {
// CHECK:           %[[TO:.*]]:4 = vector.to_elements %[[ARG1]] : vector<2x2xf32>
// CHECK:           %[[FROM0:.*]] = vector.from_elements %[[TO]]#0, %[[TO]]#1 : vector<2xf32>
// CHECK:           ptr.store %[[FROM0]], %[[ARG0]] : vector<2xf32>, !ptr.ptr<#amdgcn.addr_space<global, read_write>>
// CHECK:           %[[FROM1:.*]] = vector.from_elements %[[TO]]#2, %[[TO]]#3 : vector<2xf32>
// CHECK:           %[[TOFF:.*]] = ptr.type_offset f32 : index
// CHECK:           %[[APPLY:.*]] = affine.apply #[[$MAP0]](){{\[}}%[[TOFF]]]
// CHECK:           %[[PTR:.*]] = ptr.ptr_add inbounds %[[ARG0]], %[[APPLY]] : !ptr.ptr<#amdgcn.addr_space<global, read_write>>, index
// CHECK:           ptr.store %[[FROM1]], %[[PTR]] : vector<2xf32>, !ptr.ptr<#amdgcn.addr_space<global, read_write>>
// CHECK:           return
// CHECK:         }
func.func @store(%m : memref<4x4xf32, #amdgcn.addr_space<global, read_write>>, %v: vector<2x2xf32>) {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 0 : index
  vector.store %v, %m[%c0, %c1] : memref<4x4xf32, #amdgcn.addr_space<global, read_write>>, vector<2x2xf32>
  return
}

// CHECK-LABEL:   func.func @load_comp_store(
// CHECK-SAME:      %[[ARG0:.*]]: !ptr.ptr<#amdgcn.addr_space<global, read_write>>, %[[ARG1:.*]]: vector<2x2xf32>, %[[ARG2:.*]]: index, %[[ARG3:.*]]: index) {
// CHECK:           %[[C1:.*]] = arith.constant 1 : index
// CHECK:           %[[TOFF:.*]] = ptr.type_offset f32 : index
// CHECK:           %[[APPLY0:.*]] = affine.apply #[[$MAP1]](){{\[}}%[[ARG2]], %[[ARG3]], %[[TOFF]]]
// CHECK:           %[[PTR0:.*]] = ptr.ptr_add inbounds %[[ARG0]], %[[APPLY0]] : !ptr.ptr<#amdgcn.addr_space<global, read_write>>, index
// CHECK:           %[[LOAD0:.*]] = ptr.load %[[PTR0]] : !ptr.ptr<#amdgcn.addr_space<global, read_write>> -> vector<2xf32>
// CHECK:           %[[TO0:.*]]:2 = vector.to_elements %[[LOAD0]] : vector<2xf32>
// CHECK:           %[[ADDI:.*]] = arith.addi %[[ARG2]], %[[C1]] overflow<nsw> : index
// CHECK:           %[[APPLY1:.*]] = affine.apply #[[$MAP1]](){{\[}}%[[ADDI]], %[[ARG3]], %[[TOFF]]]
// CHECK:           %[[PTR1:.*]] = ptr.ptr_add inbounds %[[ARG0]], %[[APPLY1]] : !ptr.ptr<#amdgcn.addr_space<global, read_write>>, index
// CHECK:           %[[LOAD1:.*]] = ptr.load %[[PTR1]] : !ptr.ptr<#amdgcn.addr_space<global, read_write>> -> vector<2xf32>
// CHECK:           %[[TO1:.*]]:2 = vector.to_elements %[[LOAD1]] : vector<2xf32>
...
// CHECK:           ptr.store %[[FROM0]], %[[PTR0]] : vector<2xf32>, !ptr.ptr<#amdgcn.addr_space<global, read_write>>
// CHECK:           %[[FROM1:.*]] = vector.from_elements %[[ADD2]], %[[ADD3]] : vector<2xf32>
// CHECK:           ptr.store %[[FROM1]], %[[PTR1]] : vector<2xf32>, !ptr.ptr<#amdgcn.addr_space<global, read_write>>
// CHECK:           return
// CHECK:         }
func.func @load_comp_store(%m : memref<4x4xf32, #amdgcn.addr_space<global, read_write>>, %in: vector<2x2xf32>, %i: index, %j: index) {
  %v = vector.load %m[%i, %j] : memref<4x4xf32, #amdgcn.addr_space<global, read_write>>, vector<2x2xf32>
  %r1 = arith.addf %v, %in : vector<2x2xf32>
  vector.store %r1, %m[%i, %j] : memref<4x4xf32, #amdgcn.addr_space<global, read_write>>, vector<2x2xf32>
  return
}
...
// CHECK-SAME:      %[[ARG0:.*]]: !ptr.ptr<#amdgcn.addr_space<global, read_write>>) -> index {
// CHECK:           %[[C4:.*]] = arith.constant 4 : index
// CHECK:           return %[[C4]] : index
// CHECK:         }
func.func @test_dim_static(%m : memref<4x8xf32, #amdgcn.addr_space<global, read_write>>) -> index {
  %c0 = arith.constant 0 : index
  %dim = memref.dim %m, %c0 : memref<4x8xf32, #amdgcn.addr_space<global, read_write>>
  return %dim : index
}

// CHECK-LABEL:   func.func @test_dim_dynamic(
// CHECK-SAME:      %[[ARG0:.*]]: !ptr.ptr<#amdgcn.addr_space<global, read_write>>, %[[ARG1:.*]]: index, %[[ARG2:.*]]: index) -> index {
// CHECK:           return %[[ARG1]] : index
// CHECK:         }
func.func @test_dim_dynamic(%m : memref<4x?xf32, #amdgcn.addr_space<global, read_write>>) -> index {
  %c1 = arith.constant 1 : index
  %dim = memref.dim %m, %c1 : memref<4x?xf32, #amdgcn.addr_space<global, read_write>>
  return %dim : index
}

// CHECK-LABEL:   func.func @test_extract_metadata_static(
// CHECK-SAME:      %[[ARG0:.*]]: !ptr.ptr<#amdgcn.addr_space<global, read_write>>) -> (index, index, index, index, index) {
...
// CHECK:         }
func.func @test_extract_metadata_static(%m : memref<4x8xf32, #amdgcn.addr_space<global, read_write>>) -> (index, index, index, index, index) {
  %base, %offset, %size0, %size1, %stride0, %stride1 = memref.extract_strided_metadata %m
      : memref<4x8xf32, #amdgcn.addr_space<global, read_write>> -> memref<f32, #amdgcn.addr_space<global, read_write>>, index, index, index, index, index
...
// CHECK-SAME:      %[[ARG0:.*]]: !ptr.ptr<#amdgcn.addr_space<global, read_write>>, %[[ARG1:.*]]: index, %[[ARG2:.*]]: index, %[[ARG3:.*]]: index) -> (index, index, index, index, index) {
// CHECK:           %[[C0:.*]] = arith.constant 0 : index
// CHECK:           %[[C1:.*]] = arith.constant 1 : index
// CHECK:           return %[[C0]], %[[ARG1]], %[[ARG2]], %[[ARG3]], %[[C1]] : index, index, index, index, index
// CHECK:         }
func.func @test_extract_metadata_dynamic(%m : memref<?x?xf32, #amdgcn.addr_space<global, read_write>>) -> (index, index, index, index, index) {
  %base, %offset, %size0, %size1, %stride0, %stride1 = memref.extract_strided_metadata %m
      : memref<?x?xf32, #amdgcn.addr_space<global, read_write>> -> memref<f32, #amdgcn.addr_space<global, read_write>>, index, index, index, index, index
  return %offset, %size0, %size1, %stride0, %stride1 : index, index, index, index, index
}

// CHECK-LABEL:   func.func @test_extract_metadata_strided(
// CHECK-SAME:      %[[ARG0:.*]]: !ptr.ptr<#amdgcn.addr_space<global, read_write>>, %[[ARG1:.*]]: index, %[[ARG2:.*]]: index) -> (index, index, index, index, index) {
// CHECK:           %[[C4:.*]] = arith.constant 4 : index
// CHECK:           %[[C8:.*]] = arith.constant 8 : index
// CHECK:           %[[C1:.*]] = arith.constant 1 : index
// CHECK:           return %[[ARG1]], %[[C4]], %[[C8]], %[[ARG2]], %[[C1]] : index, index, index, index, index
// CHECK:         }
func.func @test_extract_metadata_strided(%m : memref<4x8xf32, strided<[?, 1], offset: ?>, #amdgcn.addr_space<global, read_write>>) -> (index, index, index, index, index) {
  %base, %offset, %size0, %size1, %stride0, %stride1 = memref.extract_strided_metadata %m
      : memref<4x8xf32, strided<[?, 1], offset: ?>, #amdgcn.addr_space<global, read_write>> -> memref<f32, #amdgcn.addr_space<global, read_write>>, index, index, index, index, index
  return %offset, %size0, %size1, %stride0, %stride1 : index, index, index, index, index
}

// CHECK-LABEL:   func.func @test_load_static(
// CHECK-SAME:      %[[ARG0:.*]]: !ptr.ptr<#amdgcn.addr_space<global, read_write>>, %[[ARG1:.*]]: index, %[[ARG2:.*]]: index) -> f32 {
// CHECK:           %[[TOFF:.*]] = ptr.type_offset f32 : index
// CHECK:           %[[APPLY:.*]] = affine.apply #[[$MAP2]](){{\[}}%[[ARG1]], %[[ARG2]], %[[TOFF]]]
// CHECK:           %[[PTR:.*]] = ptr.ptr_add inbounds %[[ARG0]], %[[APPLY]] : !ptr.ptr<#amdgcn.addr_space<global, read_write>>, index
// CHECK:           %[[LOAD:.*]] = ptr.load %[[PTR]] : !ptr.ptr<#amdgcn.addr_space<global, read_write>> -> f32
// CHECK:           return %[[LOAD]] : f32
// CHECK:         }
func.func @test_load_static(%m : memref<4x8xf32, #amdgcn.addr_space<global, read_write>>, %i: index, %j: index) -> f32 {
  %r = memref.load %m[%i, %j] : memref<4x8xf32, #amdgcn.addr_space<global, read_write>>
  return %r : f32
}

// CHECK-LABEL:   func.func @test_load_dynamic(
// CHECK-SAME:      %[[ARG0:.*]]: !ptr.ptr<#amdgcn.addr_space<global, read_write>>, %[[ARG1:.*]]: index, %[[ARG2:.*]]: index, %[[ARG3:.*]]: index, %[[ARG4:.*]]: index, %[[ARG5:.*]]: index) -> f32 {
// CHECK:           %[[TOFF:.*]] = ptr.type_offset f32 : index
...
// CHECK:           return %[[LOAD]] : f32
// CHECK:         }
func.func @test_load_dynamic(%m : memref<?x?xf32, #amdgcn.addr_space<global, read_write>>, %i: index, %j: index) -> f32 {
  %r = memref.load %m[%i, %j] : memref<?x?xf32, #amdgcn.addr_space<global, read_write>>
  return %r : f32
}
...
// CHECK:           return
// CHECK:         }
func.func @test_store_static(%m : memref<4x8xf32, #amdgcn.addr_space<global, read_write>>, %v: f32, %i: index, %j: index) {
  memref.store %v, %m[%i, %j] : memref<4x8xf32, #amdgcn.addr_space<global, read_write>>
  return
}
